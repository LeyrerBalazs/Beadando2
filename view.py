...
class Window:
    ...
    def WriteUnits(self, frame:tk.Frame, image_y:int) -> None:
        ...
        """
        self.WriteImgs(frame, image_y, ["imgs/spearman.jpg", "imgs/swordman.jpg", "imgs/muskater.jpg", "imgs/lighthorse.jpg", "imgs/armoredhorse.jpg"], 50)

    def WriteUnits_InAttack(self, frame:tk.Frame, image_y:int, label_y:int, user:object) -> None:  
    ...
    def WriteUnits_InLoggined(self, frame, image_y, label_y, label_y2, button_y, user, w, h) -> None:
        ...
        Args:
            ...
            h (int): Gombmagassága
        """
        types = ["spear", "sword", "musket", "light", "armored"]
        xloc_1 = 100
        xloc_2 = 50
        for id, unit in enumerate(user.units):
            tk.Label(frame, text=f'{unit.amount}', bg=self.bg_color).place(x=xloc_1, y=label_y)
            tk.Button(frame, text="+", height=h, width=w, command=functools.partial(self._Unit_Event, types[id], frame)).place(x=xloc_2, y=button_y)
    ...
    def WriteBuilds(self, frame:tk.Frame):
        ...
        xloc_1 = 100
        xloc_2 = 50
        self.WriteImgs(frame, 150, ["imgs/farm.jpg", "imgs/mine.jpg"], 50)
        types = ["farm", "goldmine"]
        for id, build in enumerate(self.u.builds):
            tk.Label(frame, text=f'{build.level}', bg=self.bg_color).place(x=xloc_1, y=160)
            tk.Button(frame, text="+", height=1, width=3, command=functools.partial(self._Building_Event, types[id], frame)).place(x=xloc_2, y=180)
    ...
    def Attack_Frame(self):
        ...
        """
        enemy = user.ActiveUser("Enemy", "")
        enemy.make_enemy_stats(self.u.score)
        ...
        self.WriteUnits_InAttack(attack_frame, 100, 110, self.u)
